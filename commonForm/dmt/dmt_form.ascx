...
<table border="0" class="bluetable" cellspacing="1" cellpadding="2" style="font-size: 9pt" width="100%">
	...
	<tr>
		...
		<td class=whitetablebg colspan=7>
			<input type=radio name=tfzy_S_Mark value="" class="Lock">商標
			<span id="smark">
			    <input type=radio name=tfzy_S_Mark value="S" class="Lock">92年修正前服務標章
			</span>
			<span id="smark1">
			    <span id="fr_smark1" style="display:">
	...
	<tr>
		...
		<td class=whitetablebg colspan=7>
            <SELECT NAME=tfzy_Pul class="Lock">
	...
	<tr style="display:" id=fileupload>
		...
		<td class=whitetablebg colspan=7>	
			<img border="0" src="../images/annex.gif" WIDTH="18" HEIGHT="18"></a>
	...
	<tr>
		...
		<td class=whitetablebg colspan=7>共<input type="text" class="Lock" name=tfzr_class_count size=2 onchange="vbscript:add_button me.value">類<input type="text" name=tfzr_class class="Lock" readonly>
	...
	<tr>
		...
		<td class="whitetablebg" colspan="7"><!--2013/1/22玉雀告知不顯示商標法施行細則第13條-->第<INPUT type="text" class="Lock" name=class11 size=2 maxlength=2 onchange="vbscript:count_kind reg.class11.value,1">類</td>		
	</tr>
    ...
    cust.init = function () {
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                //if Draw_file <> ""	then
                //    if instr(draw_file,"\")>0 then
			 	//        aa=right(Draw_file,len(Draw_file)-InstrRev(Draw_file,"\"))
			 	//        if Instr(Draw_file,"temp") = 0 then
				//	    dd=split(Draw_file,"\")
				//	    file_name="/btbrt/" & branch & "T/" & dd(4) & "/" & dd(5) & "/" & aa
                //    else
                //        file_name="/btbrt/" & branch & "T/temp/" & aa
                //    end if
                //    else
                //    file_name=draw_file
                //        end if	 
                //            <a href="http://<%=uploadserver_name%><%=file_name%>" target="blank">
                //end if
                ...
                $("#pfzd_prior_date").val(j.prior_date);
                $("#tfzd_prior_no").val(j.prior_no);
                $("#tfzd_apply_date").val(j.apply_date);
                $("#tfzd_issue_date").val(j.issue_date);
                $("#tfzd_open_date").val(j.open_date);
                $("#tfzd_rej_no").val(j.rej_no);
                $("#tfzd_end_date").val(j.end_date);
