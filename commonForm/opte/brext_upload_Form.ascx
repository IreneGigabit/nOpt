<%@ Control Language="C#" ClassName="brext_upload_form" %>
    ...
    <thead>
	    <TR>
		    <TD align=center colspan=5 class=lightbluetable1>
		        ...
		        <input type="button" value="重新抓取區所上傳文件" class="c1button ALock"  id="GetBranch_attach_button" name="GetBranch_attach_button">
		    </TD>
	    </TR>
    </thead>
    <tfoot style="display:none">
		<TR>
			...
			<TD class=sfont9 colspan="2" align="left">
	            ...
	            <input type='hidden' id='uploadservername_##' name='uploadservername_##'>
...
<script language="javascript" type="text/javascript">
    ...
    brupload_form.init = function () {
        ...
        $("#attach_seq").val(br_opte.opte[0].seq);
        $("#attach_seq1").val(br_opte.opte[0].seq1);
        $("#attach_in_no").val(br_opte.opte[0].in_no);
        $("#attach_case_no").val(br_opte.opte[0].case_no);
    }
    ...
    $("#GetBranch_attach_button").click(function () {
        if (confirm("是否確定重新取得區所上傳文件資料？")) {
            var url = getRootPath() + "/ajax/get_branchdata.aspx?prgid=<%=prgid%>&datasource=attach&branch=" + $("#Branch").val() +
