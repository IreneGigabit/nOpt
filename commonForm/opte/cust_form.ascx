<%@ Control Language="C#" ClassName="ext_cust_form" %>
...
<TR>
	...
	<input TYPE="text" id="F_cust_seq" name="F_cust_seq" size="6"  class="MLock">
        <input type="button" value="重新抓取區所客戶及聯絡人資料" class="c1button ALock"  id="GetBranch_cust_button" name="GetBranch_cust_button">
	</TD>
...
<script language="javascript" type="text/javascript">
    ...
    $("#GetBranch_cust_button").click(function () {
        if (confirm("是否確定重新取得區所案件客戶及聯絡人資料？")) {
            var url = getRootPath() + "/ajax/get_branchdata.aspx?prgid=<%=prgid%>&datasource=cust&branch=" + $("#Branch").val() + "&case_no=" + $("#case_no").val() +
