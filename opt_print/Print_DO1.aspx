...
<script runat="server">
	...
	protected void WordOut() {
        string docFileName = "[異議]-" + ipoRpt.Seq + ".docx";

		Dictionary<string, string> _tplFile = new Dictionary<string, string>(){
			{"apply", Server.MapPath("~/ReportTemplate/申請書/301異議申請書DO1.docx")},
        ...
        if (opt.Rows.Count > 0) {
            ...
            string line1="",line2="";
            var pul1 = ipoRpt.TranListE.Where(a => a["mod_field"].ToString() == "mod_pul" && a["mod_type"].ToString().Trim().Left(1) != "O");
            ...
            var pul2 = ipoRpt.TranListE.Where(a => a["mod_field"].ToString() == "mod_pul" && a["mod_type"].ToString().Trim().Left(1) == "O");
            foreach (var r in pul2) {
                if (r["mod_type"].ToString().Trim() == "O1") {
                    line2 += "\n" + line1 + "註冊應予撤銷。";
                } else if (r["mod_type"].ToString().Trim() == "O2") {
                    line2 += "\n" + line1 + string.Format("指定使用於商標法施行細則第{0}條第{1}類商品／服務之註冊應予撤銷。", r["new_no"].ToString().Trim(), r["mod_dclass"].ToString().Trim());
                } else if (r["mod_type"].ToString().Trim() == "O3") {
            ...
            //    ipoRpt.ReplaceBookmark("ncname1_aprep", r["ncname1"].ToString());
            //    ipoRpt.ReplaceBookmark("new_no_aprep", r["new_no"].ToString());
            //}
            //主張條款
            ipoRpt.CopyBlock("b_aprep");
