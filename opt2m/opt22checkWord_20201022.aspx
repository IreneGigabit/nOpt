    ...
    private void Page_Load(System.Object sender, System.EventArgs e) {
        ...
        if (dt.Rows.Count == 0) {
            ...
            try {
                ...
                //20191017 增加檢查申請書檔名
                SQL = " select * from br_opt a ";
                SQL += "where opt_sqlno = '" + Request["opt_sqlno"] + "' ";
                string brseq = "";
                using (SqlDataReader dr1 = conn.ExecuteReader(SQL)) {
                    if (dr1.Read()) {
                        brseq = dr1.SafeRead("Branch", "") + "T" + dr1.SafeRead("bseq", "") + (dr1.SafeRead("bseq1", "") != "_" ? "-" + dr1.SafeRead("bseq1", "") : "");
                    }
                }
                ...
                using (SqlDataReader dr1 = conn.ExecuteReader(SQL)) {
                    if (!dr1.Read()) {
                        strOut.AppendLine("	errFlag=true;");
                        strOut.AppendLine("	$('#msg').append('<Font align=left color=\"red\" size=3>申請書PDF檔名有誤，檔名須含有<font color=\"black\">" + required_name + "</font>及案號(<font color=\"black\">" + brseq + "</font>)，透過增益集轉檔後產生的檔名請勿任意修改!!</font><BR>');\r\n");
                        if ((Request["debug"] ?? "").ToUpper() == "Y") {
